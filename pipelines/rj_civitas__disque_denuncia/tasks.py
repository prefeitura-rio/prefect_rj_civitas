# -*- coding: utf-8 -*-
"""
Tasks for retrieving, processing, and saving Disque Denúncia XML reports (BigQuery / GCS).

Migrado de Prefect 1.4 (prefeitura_rio → iplanrio logging/bd; @task retries em segundos;
sem ENDRUN/Skipped — o flow trata lista vazia com Completed(name='Skipped')).
"""
...
from iplanrio.pipelines_utils.bd import get_project_id
...
def _normalize_secret_path(secret_path: str) -> str:
    return secret_path.upper().replace("-", "_").replace("/", "")


@task
def get_secret_folder_task(secret_path: str, inject_env: bool = False) -> dict:
    """
    Load secrets from env vars (Infisical sync → K8s). Path /foo/bar → prefix FOOBAR.
    When inject_env=True, also sets os.environ for each key (legacy parity with Prefect 1).
    """
    from os import environ

    prefix = _normalize_secret_path(secret_path)
    out: dict = {}
    prefix_with_sep = f"{prefix}__"
    for key, value in environ.items():
        if key.startswith(prefix_with_sep):
            short = key[len(prefix_with_sep) :]  # e.g. GOOGLE_MAPS_API_KEY
            out[short] = value
            if inject_env:
                environ[key] = value
    if not out:
        log(
            msg=f"No env vars found for secret path {secret_path!r} (expected keys like {prefix_with_sep}*)",
            level="warning",
        )
    return out
...
def get_reports(
    ...
    url = getenv_or_action("DISQUE_DENUNCIA__URL") + f"/difusao_{tipo_difusao.lower()}/"
...
def capture_reports(
    ...
    url = getenv_or_action("DISQUE_DENUNCIA__URL") + f"/capturadas_{tipo_difusao.lower()}/"
...
def update_missing_coordinates_in_bigquery(
    ...
    updated_rows = 0
    lat_col = lat_lon_columns_names["latitude"]
    lon_col = lat_lon_columns_names["longitude"]
    ...
    if updates:
        ...
        struct_list = []
        for row in updates:
            id_sql = str(row[id_column_name]).replace("'", "''")
            ts_sql = ""
            if timestamp_creation_column_name and timestamp_creation_column_name in row:
                ts_sql = (
                    f", DATETIME('{row[timestamp_creation_column_name]}') as `{timestamp_creation_column_name}`"
                )
            struct_list.append(
                f"""STRUCT(
                    '{id_sql}' as `{id_column_name}`,
                    {row[lat_col]} as `{lat_col}`,
                    {row[lon_col]} as `{lon_col}`{ts_sql}
                )"""
            )
        ...
        ts_update = ""
        if timestamp_creation_column_name:
            ts_update = f", target.`{timestamp_creation_column_name}` = source.`{timestamp_creation_column_name}`"
        ...
        ON target.`{id_column_name}` = source.`{id_column_name}`
        WHEN MATCHED THEN
            UPDATE SET
                target.`{lat_col}` = source.`{lat_col}`,
                target.`{lon_col}` = source.`{lon_col}`{ts_update}
        """
